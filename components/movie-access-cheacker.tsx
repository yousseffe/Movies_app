    ...
    const checkAccess = async () => {
      try {
        ...
        try {
          ...
          console.error("Error checking if movie was requested:", error)
          setRequested(false)
        }
      } catch (error) {
  ...
  const handleRequestAccess = async () => {
    if (!movieId || !movieName) {
      toast({
        ...
        description: "Missing movie information. Please try again.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/movies/${movieId}?requestSuccess=true`)
      } else {
        toast({
          title: "Request Failed",
