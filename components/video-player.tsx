...
export default function VideoPlayer({ videos = [], movieCover = "" }: VideoPlayerProps) {
  ...
  const getVideoType = (url: string) => {
    ...
    try {
      ...
      return { type: "unknown", id: null, embedUrl: null, platform: "unknown" }
    }
  }
          ...
          {safeVideos.length > 0 ? (
            safeVideos.map((video, index) => {
              ...
              return (
                <div
                  key={video._id || index}
                  className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-md border p-4"
                >
                  <div>
                    <h4 className="font-medium">{video.title || `Video ${index + 1}`}</h4>
                    <p className="text-sm text-muted-foreground break-all line-clamp-1">{video.url}</p>
